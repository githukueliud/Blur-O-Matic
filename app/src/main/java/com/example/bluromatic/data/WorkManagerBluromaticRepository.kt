...
class WorkManagerBluromaticRepository(context: Context) : BluromaticRepository {
    ...
    val imageUri: Uri = context.getImageUri()
    ...
    override fun applyBlur(blurLevel: Int) {
        ...
        //work request to clean up temporary images
        var continuation = workManager.beginWith(OneTimeWorkRequest.Companion.from(CleanupWorker::class.java))

        //input data object
        blurBuilder.setInputData(createInputDataForWorkRequest(blurLevel, imageUri))

        //start the work by calling enqueue on work manager
        //workManager.enqueue(blurBuilder.build())
        continuation = continuation.then(blurBuilder.build())
        //work request to save the image to the file system
